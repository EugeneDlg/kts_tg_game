...
class Rabbitmq:
    ...
    async def publish(self, message: str, queue) -> None:
        print(f"Publish for {queue}:: {message}")
        async with self.channel_pool.acquire() as channel:
            await channel.default_exchange.publish(
                Message(message.encode()), queue
            )

    async def consume(self, queue) -> Message:
        ...
        async with self.channel_pool.acquire() as channel:
            ...
            async with queue.iterator() as queue_iter:
                async for message in queue_iter:
                    ...
                    await message.ack()
                    print(f"CONSUME on {queue}::{ret_mes}")
