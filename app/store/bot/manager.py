...
class BotManager:
    ...
    def make_message(self, text: str, peer_id: int, user_id: int = None,
                     keyboard: dict = None, event_data: dict = None, event_id: str = None):
        if keyboard is not None:
            ...
            keyboard = json.dumps(self.build_keyboard(buttons=buttons,
                                                      inline=inline,
                                                      one_time=one_time))
        return Message(user_id=user_id, peer_id=peer_id,
    ...
    async def create_game_message(self, text, user_id: int, peer_id: int):
        await self.publish_message(text=text,
                                   user_id=user_id,
                                   peer_id=peer_id,
                                   keyboard={"buttons": [{"command": REGISTER,
    ...
    async def publish_start_message(self, text, user_id: int, peer_id: int):
        await self.publish_message(text=text,
                                   user_id=user_id,
                                   peer_id=peer_id,
                                   keyboard={"buttons": [{"command": START,
                                                          "label": "Начать игру"}]})

    async def publish_speaker_selection_message(self, game_id: int, peer_id: int, text: str):
        # breakpoint()
        ...
        buttons.append({"command": f"speaker{captain.vk_id}", "label": f"Капитан"})
        keyboard = {"buttons": buttons}
        await self.publish_message(text=text, peer_id=peer_id, keyboard=keyboard)
    ...
    async def spin_top_message(self, peer_id: int):
        ...
        await self.publish_message(text=text, peer_id=peer_id,
                                   keyboard={"buttons": [{"command": TOP,
                                                          "label": "Крутить волчок"}]})

    async def publish_message(self, text: str,
                              peer_id: int,
                              keyboard: dict = None,
                              user_id: int = None,
        ...
        message = self.make_message(text=text, peer_id=peer_id, keyboard=keyboard,
                                    user_id=user_id, event_data=event_data, event_id=event_id)
        await self.app.store.vk_api.publish_in_sender_queue(message)
    ...
    async def activate_top_timer(self, game_id: int, peer_id: int):
        task = asyncio.create_task(self.spin_top_and_show_answer(
            game_id=game_id,
            peer_id=peer_id
        ))
    ...
    async def spin_top_and_show_answer(self, game_id: int, peer_id: int):
        ...
        game = await self.app.store.game.get_game_by_id(id=game_id)
        question = await self.app.store.game.get_question(game.current_question_id)
    ...
    async def activate_thinking_timer(self, game_id: int, peer_id: int):
        task = asyncio.create_task(self.think_and_choose_speaker(
            game_id=game_id,
            peer_id=peer_id
        ))
    ...
    async def think_and_choose_speaker(self, game_id: int, peer_id: int):
        ...
        await self.activate_captain_timer(
            game_id=game_id, peer_id=peer_id,
            timer=captain_timer
        )

    async def activate_captain_timer(self, game_id: int, peer_id: int, timer: int):
        task = asyncio.create_task(self.wait_and_continue(
            game_id=game_id,
            peer_id=peer_id,
            timer=timer
        ))
    ...
    async def activate_answer_timer(self, game_id: int, peer_id: int, timer: int):
        task = asyncio.create_task(self.wait_and_continue(
            game_id=game_id,
            peer_id=peer_id,
            timer=timer)
    ...
    async def wait_and_continue(self, game_id: int, peer_id: int, timer: int):
        ...
        await self.app.store.game.update_game(id=game_id, **params)
        text = f"К сожалению, время истекло. Очко за этот раунд переходит мне. " \
    ...
    async def register_handler(self, event: Event):
        ...
        if game is not None:
            raise GameException("Некорректная команда. Игра уже идёт", event_answer=True)
        ...
        # if game has been already created, we add a player to it
            ...
            # if a player is new at all, or he was added but to another game
                ...
                if len(game.players) == self.app.config.game.players:
                    ...
                    await self.app.store.game.update_game(id=game.id, **params)
                    text = f"{full_name}, Вы зарегистрированы. " \
                    ...
                    await self.publish_message(text=text,
                                               ...
                                               event_id=event.event_id,
                                               event_data={"type": "show_snackbar"})
                    text += f" Капитаном выбран: {captain.name} {captain.last_name}. " \
                            "Он будет назначать отвечающего на вопрос в каждом раунде. " \
                            "Вы готовы? Капитан нажимает кнопку старта."
                    await self.publish_start_message(text=text,
                                                     user_id=event.user_id,
                                                     peer_id=event.peer_id)
                else:
                    await self.publish_message(text=f"{full_name}, "
                                                    "Вы зарегистрированы."
    ...
    async def start_handler(self, event: Event):
        ...
        player = await self.app.store.game.get_player_by_vk_id(vk_id=event.user_id)
        if player is None:
            raise GameException(f"{full_name}, вы не зарегистрированы как игрок!",
                                event_answer=True)
        ...
        if game.round == 0:
            ...
            await self.publish_message(text=text, user_id=event.user_id,
                                       peer_id=event.peer_id, keyboard={})
    ...
    async def speaker_handler(self, event: Event):
        ...
        if game.wait_status == CAPTAIN:
            ...
            text = f"На вопрос отвечает{captain_title} {speaker.name} {speaker.last_name}. " \
                   f"На ответ у вас есть {self.get_literal_time(self.app.config.game.answer_timer)}"
            await self.publish_message(text=text, peer_id=event.peer_id, keyboard={})
            params = {"wait_status": ANSWER, "wait_time": 0, "speaker": speaker}
            await self.app.store.game.update_game(id=game.id, **params)
            await self.activate_answer_timer(game_id=game.id, peer_id=event.peer_id,
    ...
    async def again_game_handler(self, event):
        ...
        await self.publish_message(text=text, peer_id=event.peer_id, keyboard=keyboard)
    ...
    async def top_handler(self, event: Event):
        ...
        captain = await self.app.store.game.get_captain(id=game.id)
        if captain.vk_id != event.user_id:
            raise GameException(f"{full_name}, Вы не капитан, поэтому не можете крутить волчок",
                                event_answer=True)
        round_ += 1
        ...
        await self.app.store.game.update_game(id=game.id, **params)
        await self.app.store.game.mark_question_as_used(game_id=game.id, question_id=question_id)
        await self.publish_message(text="Волчок выбирает вопрос...", peer_id=event.peer_id, keyboard={})
    ...
    async def hello_message_handler(self, message: Message):
        ...
        if game is None:
            ...
            await self.create_game_message(text=text,
            ...
            if message.user_id in [player.vk_id for player in players]:
                ...
                await self.publish_message(text=text, user_id=message.user_id, peer_id=message.peer_id)
            else:
    ...
    async def answer_message_handler(self, message):
        ...
        if game.wait_status == THINKING:
            reminder = game.wait_time + self.app.config.game.thinking_timer - int(time.time())
            raise GameException(f"{full_name}, ещё идёт обсуждение. Осталось {self.get_literal_time(reminder)}")
        if game.wait_status not in [ANSWER, EXPIRED]:
            return
        speaker = await self.app.store.game.get_speaker(id=game.id)
        current_question = await self.app.store.game.get_question(game.current_question_id)
        if speaker.vk_id != message.user_id:
            raise GameException(f"{full_name}, Вы не назначены отвечающим на вопрос")
        if game.wait_status == EXPIRED:
            raise GameException(f"К сожалению, время истекло. Вы не успели ответить.")
        if game.wait_status == ANSWER:
            ...
            if text not in current_question.answer[0].text.lower():
                ...
                await self.app.store.game.update_game(id=game.id, **params)
                text = f"К сожалению, вы ответили неправильно. Очко за этот раунд переходит мне. " \
                       f"Счёт {new_my_points}:{game.players_points}"
                await self.publish_message(text=text, peer_id=message.peer_id)
                if new_my_points == self.app.config.game.max_points:
                    await self.finish_game(game_id=game.id, peer_id=message.peer_id, winner="me")
                    return
            else:
                ...
                text = f"Вы совершенно правы!. Очко за этот раунд достаётся вам. " \
                       f"Счёт {game.my_points}:{new_your_points}"
                await self.publish_message(text=text, peer_id=message.peer_id)
                if new_your_points == self.app.config.game.max_points:
                    await self.finish_game(game_id=game.id, peer_id=message.peer_id, winner="you")
                    return
    ...
    async def finish_game(self, game_id: int, peer_id: int, winner: str):
        ...
        for player in game.players:
            points = await self.app.store.game.get_score_by_player(player_id=player.id, game_id=game_id)
    ...
    async def update_score(self, game_id: int):
        ...
        for player in game.players:
            points = await self.app.store.game.get_score_by_player(player_id=player.id, game_id=game_id)
            points += 1
            await self.app.store.game.update_player_score(player_id=player.id, game_id=game_id, points=points)
